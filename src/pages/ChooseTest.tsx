    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="space-y-3">
        ...
        {tests.map(t => (
          <button
            key={t.id}
            className={`card p-4 w-full text-left flex items-center justify-between transition-colors ${active === t.id ? 'ring-2 ring-sky-500' : ''}`}
            onClick={() => {
          ...
          >
            ...
            <div className={`size-3 rounded-full ${active === t.id ? 'bg-sky-500' : 'bg-slate-300 dark:bg-slate-700'}`} />
          </button>
        ))}
      </div>
